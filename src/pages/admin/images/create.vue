<template>
  <n-card title="Trang Cập Nhật Hình Nền" class="upload-background">
    <form @submit.prevent="submitForm">
      <div class="mb-3 mt-4">
        <label for="backgroundImages" class="form-label me-2"
          >Chọn tối đa 10 hình</label
        >
        <input
          type="file"
          id="backgroundImages"
      ...
      <div v-if="backgroundImages.length > 0" class="mb-3">
        ...
        >
          <div class="mb-2">
            <label :for="'backgroundName-' + index" class="form-label me-2"
              >Background Name {{ index + 1 }}</label
            >
            <input
              type="text"
...
const submitForm = async () => {
  ...
  backgroundImages.value.forEach((image) => {
    formData.append('background_names[]', image.name);
    formData.append('background_images[]', image.file);
  });
