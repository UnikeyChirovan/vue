...
const applySettings = () => {
  ...
  } else if (screenMode.value === 'partial') {
    reset();
  ...
  } else if (screenMode.value === 'custom') {
    reset();
    backgroundImageContainer.style.width = '66.666667%';
    backgroundImageContainer.style.backgroundImage = `url(${settings.backgroundImage})`;
    if (settings.backgroundStyle === 'solid') {
  ...
  if (settings.mode === 'night') {
    ...
    parentComponent.style.color = '#ffffff';
  } else {
    if (screenMode.value === 'full') {
      parentComponent.style.backgroundColor = settings.backgroundColor; 
    } else if (screenMode.value === 'partial') {
    ...
    parentComponent.style.color = '#000000'; 
  }
...
const getGradient = (gradientName, opacity) => {
  const gradientOpacity = `rgba(255, 255, 255, ${opacity})`; // Ví dụ sử dụng white cho opacity, bạn có thể thay đổi màu theo ý
  switch (gradientName) {
    case 'netflix':
